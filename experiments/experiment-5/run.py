#!/usr/bin/env python3
"""
Experiment 5 — MTG Knowledge Complex: Structural Conflict (Remote Flexo)

Same structural conflict scenario as Experiments 3–4 (remove BG vs enrich BG),
but run against the remote Flexo SysML v2 REST API using JSON elements.

What is being tested:
  Can the SysML v2 REST API store non-SysML (MTG) elements as JSON and
  detect the same structural conflicts as the SPARQL-based Experiments 3–4?
...
PROJECT_NAME = "mtg-kc-scenario-1"
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 5 — Remote Flexo MTG-KC scenario")
    ...
    # Commit u: remove BG + dependent faces
    u_elements = apply_commit_u(ANCESTOR_ELEMENTS)
    commit_u = client.create_commit(project_id, branch_a_id, u_elements, "Commit u: remove BG edge + dependent faces")
    ...
    # branch-uv: start from branch-a (post-u), apply v
    ...
    # branch-vu: start from branch-b (post-v), apply u
    ...
    print("  Expected: orphaned elements on branch-uv,")
    print("  clean state on branch-vu (non-commutative).")
    print("============================================")
